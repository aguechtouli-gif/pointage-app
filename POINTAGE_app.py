...
# -*- coding: utf-8 -*-
"""
app.py – Application principale de pointage (interface graphique Tkinter)
Contient toutes les classes d'interface utilisateur.
Version corrigée : rapports avec jointures hiérarchiques, exports PDF sécurisés.
"""
...
# Import de la classe Database depuis le fichier séparé
...
# ------------------------------------------------------------
# Fonction dataframe_to_pdf (export PDF)
# ------------------------------------------------------------
def dataframe_to_pdf(df, titre, filename, orientation='portrait',
                     logo_path=None, pied_page=None,
                     color_conditions=None, col_widths=None):
    """
    Exporte un DataFrame pandas en PDF avec options avancées.
    """
    ...
    if logo_path and os.path.exists(logo_path):
        try:
            ...
            header_data = [[Paragraph(titre, title_style), logo]]
    ...
    if col_widths is None:
        col_widths = [2.5] * len(df.columns)
    elif isinstance(col_widths, dict):
        auto_widths = []
        for j, col in enumerate(df.columns):
            if col in col_widths:
                auto_widths.append(col_widths[col] * cm)
            else:
                max_len = max(len(str(data[i][j])) for i in range(len(data)))
                auto_widths.append(max_len * 0.15 * cm)
    ...
    if color_conditions:
        for col_name, condition, color in color_conditions:
            if col_name in df.columns:
                col_index = df.columns.get_loc(col_name) + 1
                for i, row in enumerate(data[1:], start=1):
                    try:
                        if condition(row[col_index-1]):
                            style.add('BACKGROUND', (col_index, i), (col_index, i), color)
                    except:
                        pass
    ...
    if pied_page:
        elements.append(Spacer(1, 0.5*cm))
        style_pied = getSampleStyleSheet()['Normal']
        style_pied.alignment = 1
        style_pied.fontSize = 8
        elements.append(Paragraph(pied_page, style_pied))
...
class LoginWindow:
    ...
    def charger_logo(self):
        try:
            logo_path = resource_path(os.path.join('assets', 'logo_entreprise.png'))
...
# ============================================================
# IMPORTATION DE POINTAGES (version simplifiée)
# ============================================================
class ImportPointages:
    ...
    def ouvrir(self):
        ...
        main_frame = tk.Frame(canvas, bg='white', relief=tk.RAISED, bd=2)
        main_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
        canvas.create_window((0, 0), window=main_frame, anchor="nw")
        canvas.pack(side="left", fill="both", expand=True, padx=20, pady=10)
        scrollbar.pack(side="right", fill="y")

        instr = tk.Frame(main_frame, bg='#e8f4f8')
        instr.pack(fill=tk.X, padx=10, pady=10)
        tk.Label(instr, text="📋 Format attendu :", font=('Arial', 10, 'bold'), bg='#e8f4f8').pack(anchor='w', padx=10, pady=5)
        tk.Label(instr, text="CSV ou Excel contenant deux colonnes : matricule et datetime (date+heure).",
                 bg='#e8f4f8', font=('Arial', 9)).pack(anchor='w', padx=20, pady=2)
        tk.Label(instr, text="Le matricule peut être suivi du nom/prénom (ex: '12345A DUPONT Jean').",
                 bg='#e8f4f8', font=('Arial', 9)).pack(anchor='w', padx=20, pady=2)
        tk.Label(instr, text="La datetime peut être au format 'YYYY-MM-DD HH:MM:SS' ou 'DD/MM/YYYY HH:MM:SS'.",
                 bg='#e8f4f8', font=('Arial', 9)).pack(anchor='w', padx=20, pady=2)

        file_frame = tk.Frame(main_frame, bg='white')
        file_frame.pack(fill=tk.X, padx=10, pady=15)
        tk.Label(file_frame, text="Fichier source :", font=('Arial', 10, 'bold'), bg='white').pack(anchor='w', padx=5)
        path_frame = tk.Frame(file_frame, bg='white')
        path_frame.pack(fill=tk.X)
        self.file_path = tk.StringVar()
        tk.Entry(path_frame, textvariable=self.file_path, width=50, font=('Arial', 10)).pack(side=tk.LEFT, padx=5)
        tk.Button(path_frame, text="📂 Parcourir", command=self.parcourir_fichier,
                  bg='#3498db', fg='white', width=15).pack(side=tk.LEFT, padx=5)

        map_frame = tk.LabelFrame(main_frame, text="Correspondance des colonnes", font=('Arial', 10, 'bold'), bg='white')
        map_frame.pack(fill=tk.X, padx=10, pady=10)

        tk.Label(map_frame, text="Colonne matricule :", bg='white').grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.matricule_entry = tk.Entry(map_frame, width=30)
        self.matricule_entry.grid(row=0, column=1, padx=5, pady=5)
        self.matricule_entry.insert(0, 'matricule')

        tk.Label(map_frame, text="Colonne datetime :", bg='white').grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.datetime_entry = tk.Entry(map_frame, width=30)
        self.datetime_entry.grid(row=1, column=1, padx=5, pady=5)
        self.datetime_entry.insert(0, 'datetime')

        preview_frame = tk.LabelFrame(main_frame, text="Aperçu du fichier", font=('Arial', 10, 'bold'), bg='white')
        ...
        tk.Button(btn_frame, text="✅ IMPORTER", command=self.importer,
                  ...
                  width=20, height=2).pack(side=tk.LEFT, padx=10, expand=True)
        tk.Button(btn_frame, text="❌ ANNULER", command=self.fermer,
                  bg="#e74c3c", fg='white', width=15).pack(side=tk.LEFT, padx=10, expand=True)
        ...
        try:
            ...
            for col in df.columns:
                ...
                if 'matricule' in col_lower:
                    ...
                    self.matricule_entry.insert(0, col)
                elif 'datetime' in col_lower or 'dateheure' in col_lower or 'timestamp' in col_lower:
                    self.datetime_entry.delete(0, tk.END)
                    self.datetime_entry.insert(0, col)
        except Exception as e:
    ...
    def importer(self):
        ...
        matricule_col = self.matricule_entry.get().strip()
        datetime_col = self.datetime_entry.get().strip()
        if not matricule_col or not datetime_col:
            messagebox.showwarning("Attention", "Veuillez spécifier les colonnes matricule et datetime")
            return

        mapping = {'matricule': matricule_col, 'datetime': datetime_col}

        if not messagebox.askyesno("Confirmation", "Lancer l'importation des pointages ?"):
            return

        try:
            self.preview_text.delete(1.0, tk.END)
            self.preview_text.insert(tk.END, "⏳ Importation en cours...")
            self.window.update()
...
• Total lignes: {results['total']}
• ✅ Importés: {results['importes']}
• ⚠️ Doublons: {results['doublons']}
• ❌ Erreurs: {results['erreurs']}
"""
            self.preview_text.delete(1.0, tk.END)
            self.preview_text.insert(tk.END, result_text)
            resultats=results.get('details', [])
            if resultats:
                self.preview_text.insert(tk.END, "\n📋 Détails des erreurs (max 20):\n")
                for detail in resultats[:20]:
                    self.preview_text.insert(tk.END, f"• {detail}\n")
                    
            messagebox.showinfo("Import terminé", f"{results['importes']} pointages importés, {results['erreurs']} erreurs.")
            if results['erreurs'] == 0:
                if messagebox.askyesno("Succès", "Fermer la fenêtre ?"):
                    self.fermer()
        except Exception as e:
...
class RapportRetards:
    ...
    def ouvrir(self):
        self.window = tk.Toplevel(self.parent)
        self.window.title("Rapport des retards")
        self.window.geometry("1300x750")
        self.window.transient(self.parent)
        self.window.grab_set()
        self.window.configure(bg='#f0f0f0')

        tk.Label(self.window, text="⚠️ RAPPORT DES RETARDS",
        ...
        filtres_frame = tk.LabelFrame(main_frame, text="Filtres", font=('Arial', 12, 'bold'), bg='white')
        filtres_frame.pack(fill=tk.X, padx=10, pady=10)

        row1 = tk.Frame(filtres_frame, bg='white')
        row1.pack(fill=tk.X, padx=10, pady=5)

        tk.Label(row1, text="Matricule :", bg='white', width=10, anchor='w').pack(side=tk.LEFT)
        self.matricule_var = tk.StringVar()
        self.matricule_entry = tk.Entry(row1, textvariable=self.matricule_var, width=20)
        self.matricule_entry.pack(side=tk.LEFT, padx=5)

        tk.Label(row1, text="Direction :", bg='white', width=10, anchor='w').pack(side=tk.LEFT, padx=(20,0))
        self.direction_var = tk.StringVar()
        self.direction_combo = ttk.Combobox(row1, textvariable=self.direction_var, width=20)
        self.direction_combo.pack(side=tk.LEFT, padx=5)
        self.charger_directions()

        row2 = tk.Frame(filtres_frame, bg='white')
        row2.pack(fill=tk.X, padx=10, pady=5)

        tk.Label(row2, text="Département :", bg='white', width=12, anchor='w').pack(side=tk.LEFT)
        self.departement_var = tk.StringVar()
        self.departement_combo = ttk.Combobox(row2, textvariable=self.departement_var, width=20)
        self.departement_combo.pack(side=tk.LEFT, padx=5)
        self.charger_departements()

        tk.Label(row2, text="Service :", bg='white', width=10, anchor='w').pack(side=tk.LEFT, padx=(20,0))
        self.service_var = tk.StringVar()
        self.service_combo = ttk.Combobox(row2, textvariable=self.service_var, width=20)
        self.service_combo.pack(side=tk.LEFT, padx=5)
        self.charger_services()

        row3 = tk.Frame(filtres_frame, bg='white')
        row3.pack(fill=tk.X, padx=10, pady=5)

        tk.Label(row3, text="Équipe :", bg='white', width=12, anchor='w').pack(side=tk.LEFT)
        self.equipe_var = tk.StringVar()
        self.equipe_combo = ttk.Combobox(row3, textvariable=self.equipe_var, width=20)
        self.equipe_combo.pack(side=tk.LEFT, padx=5)
        self.charger_equipes()

        row4 = tk.Frame(filtres_frame, bg='white')
        row4.pack(fill=tk.X, padx=10, pady=10)

        tk.Label(row4, text="Période du :", bg='white').pack(side=tk.LEFT)
        self.date_debut = DateEntry(row4, width=12, date_pattern='yyyy-mm-dd', background='#3498db')
        self.date_debut.pack(side=tk.LEFT, padx=5)
        self.date_debut.set_date(date.today().replace(day=1))

        tk.Label(row4, text="au :", bg='white').pack(side=tk.LEFT, padx=5)
        self.date_fin = DateEntry(row4, width=12, date_pattern='yyyy-mm-dd', background='#3498db')
        self.date_fin.pack(side=tk.LEFT, padx=5)
        self.date_fin.set_date(date.today())

        tk.Button(row4, text="🔍 Générer", command=self.generer_rapport,
                  bg='#3498db', fg='white', font=('Arial', 10, 'bold'),
                  width=15).pack(side=tk.LEFT, padx=20)

        result_frame = tk.LabelFrame(main_frame, text="Résultats", font=('Arial', 12, 'bold'), bg='white')
        result_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        tree_frame = tk.Frame(result_frame, bg='white')
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        columns = ('Matricule', 'Nom', 'Prénom', 'Direction', 'Département', 'Service', 'Équipe',
                   'Nb retards', 'Total min', 'Justifiés', 'Non justifiés', 'Pénalité (€)')
        self.tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=15)

        col_widths = [100, 120, 120, 120, 120, 120, 120, 80, 80, 80, 80, 100]
        ...
        btn_export = tk.Frame(main_frame, bg='white')
        btn_export.pack(fill=tk.X, padx=10, pady=5)
        tk.Button(btn_export, text="📄 Exporter PDF", command=self.choisir_colonnes_et_exporter,
                  bg='#e67e22', fg='white', width=15).pack(side=tk.RIGHT, padx=5)
    ...
    def charger_directions(self):
        try:
            ...
            self.direction_combo['values'] = ['Toutes'] + [d['nom'] for d in dirs]
            self.direction_combo.set('Toutes')
        except Exception as e:
            print(f"Erreur chargement directions: {e}")

    def charger_departements(self):
        try:
            depts = self.app.db.get_all_departements()
            self.departement_combo['values'] = ['Tous'] + [d['nom'] for d in depts]
            self.departement_combo.set('Tous')
        except Exception as e:
    ...
    def charger_services(self):
        try:
            ...
            self.service_combo['values'] = ['Tous'] + [s['nom'] for s in servs]
            self.service_combo.set('Tous')
        except Exception as e:
    ...
    def charger_equipes(self):
        try:
            ...
            self.equipe_combo['values'] = ['Toutes'] + [e['nom'] for e in eqs]
            self.equipe_combo.set('Toutes')
        except Exception as e:
    ...
    def generer_rapport(self):
        try:
            ...
            if debut > fin:
                messagebox.showwarning("Attention", "La date de début doit être antérieure à la date de fin")
                return
            ...
            if matricule:
                query_agents += " AND p.matricule LIKE ?"
            ...
            if not agents:
                messagebox.showinfo("Info", "Aucun agent ne correspond aux filtres")
                return
            ...
            self.resultats.sort(key=lambda x: x['total_min'], reverse=True)
    ...
    def choisir_colonnes_et_exporter(self):
        ...
        toutes_colonnes = {
            ...
            'justifie': 'Justifiés (min)',
            'non_justifie': 'Non justifiés (min)',
            'penalite': 'Pénalité (€)'
        }

        dialog = tk.Toplevel(self.window)
        dialog.title("Choisir les colonnes à exporter")
        dialog.geometry("400x500")
        dialog.transient(self.window)
        dialog.grab_set()

        tk.Label(dialog, text="Sélectionnez les colonnes à inclure dans le PDF :",
        ...
        vars_colonnes = {}
        for i, (key, label) in enumerate(toutes_colonnes.items()):
            var = tk.BooleanVar(value=True)
            vars_colonnes[key] = var
        ...
        def valider():
            colonnes = [key for key, var in vars_colonnes.items() if var.get()]
            if not colonnes:
                messagebox.showwarning("Attention", "Sélectionnez au moins une colonne")
                return
            dialog.destroy()
            self.exporter_pdf(colonnes)

        tk.Button(btn_frame, text="Exporter", command=valider,
    ...
    def exporter_pdf(self, colonnes_choisies):
        if not self.resultats:
            messagebox.showwarning("Attention", "Aucun résultat à exporter")
            return
        ...
        try:
            ...
            colonnes_existantes = [c for c in colonnes_choisies if c in df.columns]
            if not colonnes_existantes:
                messagebox.showerror("Erreur", "Aucune colonne valide à exporter")
                return
            ...
            rename_dict = {c: mapping_noms.get(c, c) for c in colonnes_existantes}
            df_export.rename(columns=rename_dict, inplace=True)

            titre = f"Rapport des retards du {self.date_debut.get_date().strftime('%d/%m/%Y')} au {self.date_fin.get_date().strftime('%d/%m/%Y')}"
            orientation = 'landscape' if len(colonnes_existantes) > 6 else 'portrait'

            logo_path = os.path.join('pointage_data', 'assets', 'logo_entreprise.png')
            if not os.path.exists(logo_path):
                logo_path = None

            user = self.app.current_user.get('prenom', '') + ' ' + self.app.current_user.get('nom', '')
            pied = f"Généré le {datetime.now().strftime('%d/%m/%Y %H:%M')} par {user}"

            color_conditions = []
            if 'Nb retards' in df_export.columns:
                color_conditions.append(('Nb retards', lambda x: x > 10, colors.lightcoral))
            if 'Pénalité (€)' in df_export.columns:
                color_conditions.append(('Pénalité (€)', lambda x: x > 50, colors.lightyellow))

            col_widths = [2.5] * len(df_export.columns)
            ...
            messagebox.showinfo("Succès", f"Export PDF terminé vers {filename}")
        except Exception as e:
...
class RapportAbsences:
    ...
    def ouvrir(self):
        self.window = tk.Toplevel(self.parent)
        self.window.title("Rapport des absences")
        self.window.geometry("1300x750")
        self.window.transient(self.parent)
        self.window.grab_set()
        self.window.configure(bg='#f0f0f0')

        tk.Label(self.window, text="📅 RAPPORT DES ABSENCES",
                font=('Arial', 16, 'bold'), bg='#f0f0f0', fg='#2c3e50').pack(pady=10)
        ...
        columns = ('Matricule', 'Nom', 'Prénom', 'Direction', 'Département', 'Service', 'Équipe',
                   'Jours ouvrés', 'Jours pointés', 'Jours congé', 'Absences')
        self.tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=15)

        col_widths = [100, 120, 120, 120, 120, 120, 120, 100, 100, 100, 100]
        for col, w in zip(columns, col_widths):
            self.tree.heading(col, text=col)
            self.tree.column(col, width=w)
    ...
    def est_weekend(self, d):
        return self.app.db.est_weekend(d)
    ...
    def jours_ouvres(self, debut, fin):
        jour = debut
        while jour <= fin:
    ...
    def generer_rapport(self):
        try:
            ...
            self.resultats = []
            for agent in agents:
                jours_ouvres_list = list(self.jours_ouvres(debut, fin))
    ...
    def exporter_pdf(self, colonnes_choisies):
        ...
        try:
            ...
            titre = f"Rapport des absences du {self.date_debut.get_date().strftime('%d/%m/%Y')} au {self.date_fin.get_date().strftime('%d/%m/%Y')}"
            orientation = 'landscape' if len(colonnes_existantes) > 6 else 'portrait'
            ...
            color_conditions = []
            if 'Absences' in df_export.columns:
                color_conditions.append(('Absences', lambda x: x > 5, colors.lightcoral))
...
class RapportPresence:
    ...
    def ouvrir(self):
        self.window = tk.Toplevel(self.parent)
        self.window.title("Rapport de présence")
        self.window.geometry("1300x750")
        self.window.transient(self.parent)
        self.window.grab_set()
        self.window.configure(bg='#f0f0f0')

        tk.Label(self.window, text="📋 RAPPORT DE PRÉSENCE",
                font=('Arial', 16, 'bold'), bg='#f0f0f0', fg='#2c3e50').pack(pady=10)
        ...
        tk.Label(row2, text="Département :", bg='white', width=12, anchor='w').pack(side=tk.LEFT)
        self.departement_var = tk.StringVar()
        self.departement_combo = ttk.Combobox(row2, textvariable=self.departement_var, width=15)
        self.departement_combo.pack(side=tk.LEFT, padx=5)
        self.charger_departements()

        tk.Label(row2, text="Service :", bg='white', width=10, anchor='w').pack(side=tk.LEFT, padx=(10,0))
        self.service_var = tk.StringVar()
        self.service_combo = ttk.Combobox(row2, textvariable=self.service_var, width=15)
        self.service_combo.pack(side=tk.LEFT, padx=5)
        self.charger_services()

        tk.Label(row2, text="Équipe :", bg='white', width=10, anchor='w').pack(side=tk.LEFT, padx=(10,0))
        self.equipe_var = tk.StringVar()
        self.equipe_combo = ttk.Combobox(row2, textvariable=self.equipe_var, width=15)
        self.equipe_combo.pack(side=tk.LEFT, padx=5)
        self.charger_equipes()
        ...
        tk.Label(row3, text="Période du :", bg='white').pack(side=tk.LEFT)
        self.date_debut = DateEntry(row3, width=12, date_pattern='yyyy-mm-dd', background='#3498db')
        self.date_debut.pack(side=tk.LEFT, padx=5)
        self.date_debut.set_date(date.today().replace(day=1))

        tk.Label(row3, text="au :", bg='white').pack(side=tk.LEFT, padx=5)
        self.date_fin = DateEntry(row3, width=12, date_pattern='yyyy-mm-dd', background='#3498db')
        self.date_fin.pack(side=tk.LEFT, padx=5)
        self.date_fin.set_date(date.today())

        tk.Button(row3, text="🔍 Générer", command=self.generer_rapport,
                  bg='#3498db', fg='white', font=('Arial', 10, 'bold'),
                  width=15).pack(side=tk.LEFT, padx=20)
        ...
        col_widths = [100, 120, 120, 120, 120, 120, 120, 100]
        for col, w in zip(columns, col_widths):
            self.tree.heading(col, text=col)
            self.tree.column(col, width=w)
    ...
    def generer_rapport(self):
        try:
            ...
            self.resultats.sort(key=lambda x: (x['nom'], x['prenom']))
    ...
    def exporter_pdf(self, colonnes_choisies):
        ...
        try:
            ...
            titre = f"Rapport de présence du {self.date_debut.get_date().strftime('%d/%m/%Y')} au {self.date_fin.get_date().strftime('%d/%m/%Y')}"
            orientation = 'landscape' if len(colonnes_existantes) > 6 else 'portrait'
            ...
            color_conditions = []
            if 'Jours présence' in df_export.columns:
                color_conditions.append(('Jours présence', lambda x: x > 20, colors.lightgreen))
...
class PointageApp:
    ...
    def show_main_window(self):
        ...
        self.main_window.title("Système de Pointage - Accueil")
        ...
        file_menu.add_command(label="Exporter les données", command=self.exporter_donnees)
        file_menu.add_separator()
        file_menu.add_command(label="Quitter", command=self.main_window.quit)

        # Personnel
        pers_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Personnel", menu=pers_menu)
        pers_menu.add_command(label="Liste du personnel", command=self.liste_personnel)
        pers_menu.add_command(label="Ajouter un agent", command=self.ajouter_personnel)
        pers_menu.add_separator()
        pers_menu.add_command(label="Importer fichier", command=self.importer_personnel)

        # Pointages
        pt_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Pointages", menu=pt_menu)
        pt_menu.add_command(label="Pointage rapide", command=self.pointage_rapide)
        pt_menu.add_command(label="Liste des pointages", command=self.liste_pointages)
        pt_menu.add_separator()
        pt_menu.add_command(label="Importer pointages", command=self.importer_pointages)

        # Rapports
        rep_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Rapports", menu=rep_menu)
        rep_menu.add_command(label="Rapport des retards", command=self.rapport_retards)
        rep_menu.add_command(label="Rapport des absences", command=self.rapport_absences)
        rep_menu.add_command(label="Rapport de présence", command=self.rapport_presence)

        # Paramètres
        param_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Paramètres", menu=param_menu)
        param_menu.add_command(label="Jours fériés", command=self.gestion_jours_feries)
        param_menu.add_command(label="Paramètres généraux", command=self.parametres_generaux)
        ...
        # Contenu principal
        main_frame = tk.Frame(self.main_window, bg='white', relief=tk.RAISED, bd=2)
        ...
        tk.Label(main_frame, text="BIENVENUE DANS LE SYSTÈME DE POINTAGE",
                 font=('Arial', 18, 'bold'), fg='#2c3e50', bg='white').pack(pady=20)

        tk.Label(main_frame, text=f"Connecté : {self.current_user.get('prenom', '')} {self.current_user.get('nom', '')}",
                 font=('Arial', 10), fg='#7f8c8d', bg='white').pack(pady=5)
        ...
        try:
            stats = self.db.get_statistiques_completes()
            tk.Label(stats_frame, text=f"👥 Personnel actif : {stats.get('personnel_actif', 0)}",
                     font=('Arial', 12), bg='#ecf0f1', fg='#2c3e50').pack(side=tk.LEFT, padx=20, pady=10)
            tk.Label(stats_frame, text=f"✅ Pointages ce mois : {stats.get('pointages_periode', 0)}",
                     font=('Arial', 12), bg='#ecf0f1', fg='#2c3e50').pack(side=tk.LEFT, padx=20, pady=10)
            tk.Label(stats_frame, text=f"⚠️ Retards ce mois : {stats.get('retards_periode', 0)}",
                     font=('Arial', 12), bg='#ecf0f1', fg='#2c3e50').pack(side=tk.LEFT, padx=20, pady=10)
        except:
            pass
        ...
        tk.Button(btn_frame, text="📋 Pointage rapide", command=self.pointage_rapide,
                  bg='#3498db', fg='white', font=('Arial', 11, 'bold'), width=18, height=2).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="👥 Liste personnel", command=self.liste_personnel,
                  bg='#2ecc71', fg='white', font=('Arial', 11, 'bold'), width=18, height=2).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="📊 Rapport retards", command=self.rapport_retards,
                  bg='#e67e22', fg='white', font=('Arial', 11, 'bold'), width=18, height=2).pack(side=tk.LEFT, padx=10)

        self.main_window.mainloop()

    def exporter_donnees(self):
        messagebox.showinfo("Exporter", "Fonction à implémenter")

    def liste_personnel(self):
        messagebox.showinfo("Liste personnel", "Fonction à implémenter")

    def ajouter_personnel(self):
        messagebox.showinfo("Ajouter", "Fonction à implémenter")

    def importer_personnel(self):
        messagebox.showinfo("Importer", "Fonction à implémenter")

    def pointage_rapide(self):
        matricule = simpledialog.askstring("Pointage rapide", "Entrez le matricule :")
        if matricule:
            type_pt = messagebox.askquestion("Type", "Pointage entrée ? (Oui=entrée, Non=sortie)")
            type_pt = "entrée" if type_pt == 'yes' else "sortie"
            result, msg = self.db.add_pointage_avance(matricule, type_pt, user_id=self.current_user['id'])
            messagebox.showinfo("Résultat", msg)

    def liste_pointages(self):
        messagebox.showinfo("Liste pointages", "Fonction à implémenter")

    def importer_pointages(self):
        ImportPointages(self.main_window, self).ouvrir()

    def rapport_retards(self):
        RapportRetards(self.main_window, self).ouvrir()

    def rapport_absences(self):
        RapportAbsences(self.main_window, self).ouvrir()

    def rapport_presence(self):
        RapportPresence(self.main_window, self).ouvrir()

    def gestion_jours_feries(self):
        messagebox.showinfo("Jours fériés", "Fonction à implémenter")

    def parametres_generaux(self):
        messagebox.showinfo("Paramètres", "Fonction à implémenter")

    def a_propos(self):
        messagebox.showinfo("À propos", "Système de Pointage\nVersion 2.0\n© 2025")


# ============================================================
# GESTION HIÉRARCHIE (Activités, Divisions, Directions, Départements, Services, Équipes)
# ============================================================
class GestionHierarchie:
    ...
    def ouvrir(self):
        ...
        notebook.add(self.tab_div, text="📦 Divisions")
        self.create_tab_divisions()

        self.tab_dir = ttk.Frame(notebook)
        notebook.add(self.tab_dir, text="🏢 Directions")
        self.create_tab_directions()

        self.tab_dep = ttk.Frame(notebook)
        notebook.add(self.tab_dep, text="🏬 Départements")
        self.create_tab_departements()
        ...
        # Chargement initial des listes (les combobox sont déjà créées dans les onglets)
        self.charger_activites()
        self.charger_divisions()
        self.charger_directions()
        self.charger_departements()
        self.charger_services()
        self.charger_equipes()
    ...
    def create_tab_activites(self):
        ...
        self.act_responsable = tk.Entry(add_frame, width=20)
        self.act_responsable.grid(row=0, column=5, padx=5, pady=5)
        ...
        cols = ('ID', 'Nom', 'Code', 'Responsable', 'Active')
    ...
    def ajouter_activite(self):
        ...
        if not nom:
            ...
            return
        responsable = self.act_responsable.get().strip() or None
        self.app.db.add_activite(nom, self.act_code.get() or None, responsable)
        messagebox.showinfo("Succès", f"Activité '{nom}' ajoutée")
        self.act_nom.delete(0, tk.END)
        self.act_code.delete(0, tk.END)
        self.act_responsable.delete(0, tk.END)
        self.charger_activites()
        # Rafraîchir les combobox dépendantes
    ...
    def charger_activites(self):
        ...
        for a in acts:
            self.tree_act.insert('', 'end', values=(
                a['id'],
                a['nom'],
                a.get('code_activite', '') or '',
                a.get('responsable', '') or '',
                'Oui' if a.get('active', 1) else 'Non'
            ))

    def charger_activites_combo(self):
        """Charge la liste des activités dans la combobox de l'onglet Divisions."""
        try:
            acts = self.app.db.get_all_activites()
            self.div_activite['values'] = [a['nom'] for a in acts]
        except Exception as e:
            print(f"Erreur chargement activités dans combo: {e}")

    def desactiver_activite(self):
        sel = self.tree_act.selection()
        if not sel:
            messagebox.showwarning("Attention", "Sélectionnez une activité")
            return
    ...
    # ---------- DIVISIONS ----------
    def create_tab_divisions(self):
        main = tk.Frame(self.tab_div, bg='white', relief=tk.RAISED, bd=2)
        main.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

        add_frame = tk.LabelFrame(main, text="Ajouter une division", font=('Arial',11,'bold'), bg='white')
        add_frame.pack(fill=tk.X, padx=10, pady=10)

        tk.Label(add_frame, text="Nom :", bg='white').grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.div_nom = tk.Entry(add_frame, width=25)
        self.div_nom.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Code :", bg='white').grid(row=0, column=2, padx=5, pady=5, sticky='w')
        self.div_code = tk.Entry(add_frame, width=15)
        self.div_code.grid(row=0, column=3, padx=5, pady=5)

        tk.Label(add_frame, text="Responsable :", bg='white').grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.div_responsable = tk.Entry(add_frame, width=25)
        self.div_responsable.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Activité :", bg='white').grid(row=1, column=2, padx=5, pady=5, sticky='w')
        self.div_activite = ttk.Combobox(add_frame, width=25, state='readonly')
        self.div_activite.grid(row=1, column=3, padx=5, pady=5)
        self.charger_activites_combo()  # Remplir la combobox

        tk.Label(add_frame, text="Description :", bg='white').grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.div_description = tk.Text(add_frame, height=3, width=60)
        self.div_description.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky='w')

        tk.Button(add_frame, text="➕ Ajouter", command=self.ajouter_division,
                  bg='#27ae60', fg='white').grid(row=3, column=0, columnspan=4, pady=10)

        list_frame = tk.Frame(main, bg='white')
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        cols = ('ID', 'Nom', 'Code', 'Responsable', 'Activité', 'Active')
        self.tree_div = ttk.Treeview(list_frame, columns=cols, show='headings', height=12)
        for c in cols:
            self.tree_div.heading(c, text=c)
            self.tree_div.column(c, width=120)
        vsb = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.tree_div.yview)
        self.tree_div.configure(yscrollcommand=vsb.set)
        self.tree_div.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        vsb.pack(side=tk.RIGHT, fill=tk.Y)

        btn_frame = tk.Frame(main, bg='white')
        btn_frame.pack(fill=tk.X, padx=10, pady=10)
        tk.Button(btn_frame, text="🗑️ Désactiver", command=self.desactiver_division,
                  bg='#e74c3c', fg='white').pack(side=tk.RIGHT, padx=5)

    def ajouter_division(self):
        nom = self.div_nom.get().strip()
        if not nom:
            messagebox.showwarning("Attention", "Le nom est obligatoire")
            return
        code = self.div_code.get().strip() or None
        responsable = self.div_responsable.get().strip() or None
        description = self.div_description.get("1.0", tk.END).strip() or None
        activite_nom = self.div_activite.get()
        activite_id = None
        if activite_nom:
            acts = self.app.db.get_all_activites()
            for a in acts:
                if a['nom'] == activite_nom:
                    activite_id = a['id']
                    break
        self.app.db.add_division(nom, code, responsable, description, activite_id)
        messagebox.showinfo("Succès", f"Division '{nom}' ajoutée")
        self.div_nom.delete(0, tk.END)
        self.div_code.delete(0, tk.END)
        self.div_responsable.delete(0, tk.END)
        self.div_description.delete("1.0", tk.END)
        self.div_activite.set('')
        self.charger_divisions()
        self.charger_divisions_combo()  # Rafraîchir la combobox des divisions

    def charger_divisions(self):
        for i in self.tree_div.get_children():
            self.tree_div.delete(i)
        divs = self.app.db.get_all_divisions()
        for d in divs:
            # Récupérer le nom de l'activité (si l'attribut n'existe pas, on affiche vide)
            activite_nom = d.get('activite_nom', '')
            self.tree_div.insert('', 'end', values=(
                d['id'],
                d['nom'],
                d.get('code_division', '') or '',
                d.get('responsable', '') or '',
                activite_nom,
                'Oui' if d.get('active', 1) else 'Non'
            ))

    def charger_divisions_combo(self):
        """Charge la liste des divisions dans la combobox de l'onglet Directions."""
        try:
            divs = self.app.db.get_all_divisions()
            self.dir_division['values'] = [d['nom'] for d in divs]
        except Exception as e:
            print(f"Erreur chargement divisions dans combo: {e}")

    def desactiver_division(self):
        sel = self.tree_div.selection()
        if not sel:
            messagebox.showwarning("Attention", "Sélectionnez une division")
            return
        div_id = self.tree_div.item(sel[0])['values'][0]
        with self.app.db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE divisions SET active=0 WHERE id=?", (div_id,))
            conn.commit()
        self.charger_divisions()
        self.charger_divisions_combo()
    ...
    def create_tab_directions(self):
        ...
        tk.Label(add_frame, text="Responsable :", bg='white').grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.dir_responsable = tk.Entry(add_frame, width=25)
        self.dir_responsable.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Division :", bg='white').grid(row=1, column=2, padx=5, pady=5, sticky='w')
        self.dir_division = ttk.Combobox(add_frame, width=25, state='readonly')
        self.dir_division.grid(row=1, column=3, padx=5, pady=5)
        self.charger_divisions_combo()  # Remplir la combobox

        tk.Label(add_frame, text="Description :", bg='white').grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.dir_description = tk.Text(add_frame, height=3, width=60)
        self.dir_description.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky='w')
        ...
        list_frame = tk.Frame(main, bg='white')
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        cols = ('ID', 'Nom', 'Code', 'Responsable', 'Division', 'Active')
    ...
    def ajouter_direction(self):
        ...
        if not nom:
            messagebox.showwarning("Attention", "Le nom est obligatoire")
            return
        code = self.dir_code.get().strip() or None
        responsable = self.dir_responsable.get().strip() or None
        description = self.dir_description.get("1.0", tk.END).strip() or None
        division_nom = self.dir_division.get()
        division_id = None
        if division_nom:
            divs = self.app.db.get_all_divisions()
            for d in divs:
                if d['nom'] == division_nom:
                    division_id = d['id']
                    break
        self.app.db.add_direction(nom, code, responsable, description, division_id)
        messagebox.showinfo("Succès", f"Direction '{nom}' ajoutée")
        self.dir_nom.delete(0, tk.END)
        self.dir_code.delete(0, tk.END)
        self.dir_responsable.delete(0, tk.END)
        self.dir_description.delete("1.0", tk.END)
        self.dir_division.set('')
        self.charger_directions()
        self.charger_directions_combo_dep()  # Rafraîchir la combobox des départements
    ...
    def charger_directions(self):
        ...
        for d in dirs:
            self.tree_dir.insert('', 'end', values=(
                d['id'],
                d['nom'],
                d.get('code_direction', '') or '',
                d.get('responsable', '') or '',
                d.get('division_nom', '') or '',
                'Oui' if d.get('active', 1) else 'Non'
            ))

    def charger_directions_combo_dep(self):
        """Charge la liste des directions dans la combobox de l'onglet Départements."""
        try:
            dirs = self.app.db.get_all_directions()
            self.dep_direction['values'] = [d['nom'] for d in dirs]
        except Exception as e:
            print(f"Erreur chargement directions dans combo: {e}")
    ...
    def desactiver_direction(self):
        ...
        if not sel:
            ...
            return
        dir_id = self.tree_dir.item(sel[0])['values'][0]
        with self.app.db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE directions SET active=0 WHERE id=?", (dir_id,))
            conn.commit()
        self.charger_directions()
        self.charger_directions_combo_dep()

    # ---------- DÉPARTEMENTS ----------
    def create_tab_departements(self):
        main = tk.Frame(self.tab_dep, bg='white', relief=tk.RAISED, bd=2)
        main.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

        add_frame = tk.LabelFrame(main, text="Ajouter un département", font=('Arial',11,'bold'), bg='white')
        add_frame.pack(fill=tk.X, padx=10, pady=10)

        tk.Label(add_frame, text="Nom :", bg='white').grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.dep_nom = tk.Entry(add_frame, width=25)
        self.dep_nom.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Code :", bg='white').grid(row=0, column=2, padx=5, pady=5, sticky='w')
        self.dep_code = tk.Entry(add_frame, width=15)
        self.dep_code.grid(row=0, column=3, padx=5, pady=5)

        tk.Label(add_frame, text="Responsable :", bg='white').grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.dep_responsable = tk.Entry(add_frame, width=25)
        self.dep_responsable.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Direction :", bg='white').grid(row=1, column=2, padx=5, pady=5, sticky='w')
        self.dep_direction = ttk.Combobox(add_frame, width=25, state='readonly')
        self.dep_direction.grid(row=1, column=3, padx=5, pady=5)
        self.charger_directions_combo_dep()  # Remplir la combobox

        tk.Label(add_frame, text="Description :", bg='white').grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.dep_description = tk.Text(add_frame, height=3, width=60)
        self.dep_description.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky='w')

        tk.Button(add_frame, text="➕ Ajouter", command=self.ajouter_departement,
                  bg='#27ae60', fg='white').grid(row=3, column=0, columnspan=4, pady=10)

        list_frame = tk.Frame(main, bg='white')
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        cols = ('ID', 'Nom', 'Code', 'Responsable', 'Direction', 'Active')
        self.tree_dep = ttk.Treeview(list_frame, columns=cols, show='headings', height=12)
        for c in cols:
            self.tree_dep.heading(c, text=c)
            self.tree_dep.column(c, width=120)
        vsb = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.tree_dep.yview)
        self.tree_dep.configure(yscrollcommand=vsb.set)
        self.tree_dep.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        vsb.pack(side=tk.RIGHT, fill=tk.Y)

        btn_frame = tk.Frame(main, bg='white')
        btn_frame.pack(fill=tk.X, padx=10, pady=10)
        tk.Button(btn_frame, text="🗑️ Désactiver", command=self.desactiver_departement,
                  bg='#e74c3c', fg='white').pack(side=tk.RIGHT, padx=5)

    def ajouter_departement(self):
        nom = self.dep_nom.get().strip()
        if not nom:
            messagebox.showwarning("Attention", "Le nom est obligatoire")
            return
        code = self.dep_code.get().strip() or None
        responsable = self.dep_responsable.get().strip() or None
        description = self.dep_description.get("1.0", tk.END).strip() or None
        direction_nom = self.dep_direction.get()
        direction_id = None
        if direction_nom:
            dirs = self.app.db.get_all_directions()
            for d in dirs:
                if d['nom'] == direction_nom:
                    direction_id = d['id']
                    break
        self.app.db.add_departement(nom, code, responsable, description, direction_id)
        messagebox.showinfo("Succès", f"Département '{nom}' ajouté")
        self.dep_nom.delete(0, tk.END)
        self.dep_code.delete(0, tk.END)
        self.dep_responsable.delete(0, tk.END)
        self.dep_description.delete("1.0", tk.END)
        self.dep_direction.set('')
        self.charger_departements()
        self.charger_departements_combo_serv()  # Rafraîchir la combobox des services

    def charger_departements(self):
        for i in self.tree_dep.get_children():
            self.tree_dep.delete(i)
        deps = self.app.db.get_all_departements()
        for d in deps:
            self.tree_dep.insert('', 'end', values=(
                d['id'],
                d['nom'],
                d.get('code_departement', '') or '',
                d.get('responsable', '') or '',
                d.get('direction_nom', '') or '',
                'Oui' if d.get('active', 1) else 'Non'
            ))

    def charger_departements_combo_serv(self):
        """Charge la liste des départements dans la combobox de l'onglet Services."""
        try:
            deps = self.app.db.get_all_departements()
            self.serv_departement['values'] = [d['nom'] for d in deps]
        except Exception as e:
            print(f"Erreur chargement départements dans combo: {e}")

    def desactiver_departement(self):
        sel = self.tree_dep.selection()
        if not sel:
            messagebox.showwarning("Attention", "Sélectionnez un département")
            return
        dep_id = self.tree_dep.item(sel[0])['values'][0]
        with self.app.db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE departements SET active=0 WHERE id=?", (dep_id,))
            conn.commit()
        self.charger_departements()
        self.charger_departements_combo_serv()
    ...
    def create_tab_services(self):
        ...
        tk.Label(add_frame, text="Responsable :", bg='white').grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.serv_responsable = tk.Entry(add_frame, width=25)
        self.serv_responsable.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Département :", bg='white').grid(row=1, column=2, padx=5, pady=5, sticky='w')
        self.serv_departement = ttk.Combobox(add_frame, width=25, state='readonly')
        self.serv_departement.grid(row=1, column=3, padx=5, pady=5)
        self.charger_departements_combo_serv()  # Remplir la combobox

        tk.Label(add_frame, text="Description :", bg='white').grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.serv_description = tk.Text(add_frame, height=3, width=60)
        self.serv_description.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky='w')
        ...
        list_frame = tk.Frame(main, bg='white')
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        cols = ('ID', 'Nom', 'Code', 'Responsable', 'Département', 'Active')
    ...
    def ajouter_service(self):
        ...
        if not nom:
            messagebox.showwarning("Attention", "Le nom est obligatoire")
            return
        code = self.serv_code.get().strip() or None
        responsable = self.serv_responsable.get().strip() or None
        description = self.serv_description.get("1.0", tk.END).strip() or None
        departement_nom = self.serv_departement.get()
        departement_id = None
        if departement_nom:
            deps = self.app.db.get_all_departements()
            for d in deps:
                if d['nom'] == departement_nom:
                    departement_id = d['id']
                    break
        self.app.db.add_service(nom, code, responsable, description, departement_id)
        messagebox.showinfo("Succès", f"Service '{nom}' ajouté")
        self.serv_nom.delete(0, tk.END)
        self.serv_code.delete(0, tk.END)
        self.serv_responsable.delete(0, tk.END)
        self.serv_description.delete("1.0", tk.END)
        self.serv_departement.set('')
        self.charger_services()
        self.charger_services_combo_eq()  # Rafraîchir la combobox des équipes
    ...
    def charger_services(self):
        ...
        for s in servs:
            self.tree_serv.insert('', 'end', values=(
                s['id'],
                s['nom'],
                s.get('code_service', '') or '',
                s.get('responsable', '') or '',
                s.get('departement_nom', '') or '',
                'Oui' if s.get('active', 1) else 'Non'
            ))

    def charger_services_combo_eq(self):
        """Charge la liste des services dans la combobox de l'onglet Équipes."""
        try:
            servs = self.app.db.get_all_services()
            self.eq_service['values'] = [s['nom'] for s in servs]
        except Exception as e:
            print(f"Erreur chargement services dans combo: {e}")

    def desactiver_service(self):
        sel = self.tree_serv.selection()
        if not sel:
            messagebox.showwarning("Attention", "Sélectionnez un service")
            return
        ...
        with self.app.db.get_connection() as conn:
            ...
            conn.commit()
        self.charger_services()
        self.charger_services_combo_eq()
    ...
    def create_tab_equipes(self):
        ...
        tk.Label(add_frame, text="Responsable :", bg='white').grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.eq_responsable = tk.Entry(add_frame, width=25)
        self.eq_responsable.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Service :", bg='white').grid(row=1, column=2, padx=5, pady=5, sticky='w')
        self.eq_service = ttk.Combobox(add_frame, width=25, state='readonly')
        self.eq_service.grid(row=1, column=3, padx=5, pady=5)
        self.charger_services_combo_eq()  # Remplir la combobox

        tk.Label(add_frame, text="Description :", bg='white').grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.eq_description = tk.Text(add_frame, height=3, width=60)
        self.eq_description.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky='w')
        ...
        list_frame = tk.Frame(main, bg='white')
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        cols = ('ID', 'Nom', 'Code', 'Responsable', 'Service', 'Active')
    ...
    def ajouter_equipe(self):
        ...
        if not nom:
            messagebox.showwarning("Attention", "Le nom est obligatoire")
            return
        code = self.eq_code.get().strip() or None
        responsable = self.eq_responsable.get().strip() or None
        description = self.eq_description.get("1.0", tk.END).strip() or None
        service_nom = self.eq_service.get()
        service_id = None
        if service_nom:
            servs = self.app.db.get_all_services()
            for s in servs:
                if s['nom'] == service_nom:
                    service_id = s['id']
                    break
        self.app.db.add_equipe(nom, code, responsable, description, service_id)
        messagebox.showinfo("Succès", f"Équipe '{nom}' ajoutée")
        self.eq_nom.delete(0, tk.END)
        self.eq_code.delete(0, tk.END)
        self.eq_responsable.delete(0, tk.END)
        self.eq_description.delete("1.0", tk.END)
        self.eq_service.set('')
        self.charger_equipes()
    ...
    def charger_equipes(self):
        ...
        for e in eqs:
            self.tree_eq.insert('', 'end', values=(
                e['id'],
                e['nom'],
                e.get('code_equipe', '') or '',
                e.get('responsable', '') or '',
                e.get('service_nom', '') or '',
                'Oui' if e.get('active', 1) else 'Non'
            ))

    def desactiver_equipe(self):
        sel = self.tree_eq.selection()
        if not sel:
            messagebox.showwarning("Attention", "Sélectionnez une équipe")
            return
...
# ============================================================
# Onglet 3 : Gestion des tolérances et horaires
# ============================================================
class GestionTolerances:
    ...
    def ouvrir(self):  # renommé de ouvrir_interface
        self.window = tk.Toplevel(self.parent)
    ...
    def appliquer_a_tous(self):
        if messagebox.askyesno("Confirmation", "Appliquer les tolérances par défaut à TOUS les agents ?\nCette action écrasera leurs tolérances individuelles actuelles."):
            try:
    ...
    def on_agent_select(self, event):
        ...
        if not sel:
            return
        vals = self.tree_agents.item(sel[0])['values']
        self.current_agent_id = vals[0]
        agent = self.app.db.get_personnel(self.current_agent_id)
        if not agent:
            return
        self.agent_info.config(text=f"Agent sélectionné : {agent['prenom']} {agent['nom']} (Matricule: {agent['matricule']})")
        ...
        self.quart_var.set(agent.get('type_quart', 'jour'))
        self.concerne_var.set(bool(agent.get('concerne_pointage', 1)))
    ...
    def charger_directions_analyse(self):
        try:
            dirs = self.app.db.get_all_directions()
            self.analyse_direction['values'] = ['Toutes'] + [d['nom'] for d in dirs]
...
class ImportPersonnel:
    ...
    def ouvrir(self):
        ...
        tk.Label(instr, text="Colonne obligatoire : matricule (peut contenir 'matricule nom prenom')",
                bg='#e8f4f8').pack(anchor='w', padx=20, pady=2)
        tk.Label(instr, text="Autres colonnes reconnues : badge_id, nom, prenom, type_person, fonction, direction, departement, service, equipe, date_embauche, date_naissance, adresse, telephone, email, statut, concerne_pointage, type_quart, heure_entree_theorique, heure_sortie_theorique, tolerance_entree, tolerance_sortie",
                bg='#e8f4f8', font=('Arial', 8)).pack(anchor='w', padx=20, pady=2)
    ...
    def afficher_apercu(self, fn):
        try:
            ...
            self.preview.insert(tk.END, "📋 APERÇU DES 5 PREMIÈRES LIGNES:\n\n")
...
class PersonnelDialog:
    def __init__(self, parent, app, personnel_id=None):
        ...
        self.photo_actuelle = None
        self.id_maps = {}  # pour stocker les correspondances nom -> id
        ...
        # Charger les listes AVANT de charger les données (pour avoir les mappings)
        self.charger_listes()
        self.create_widgets()
        if personnel_id:
            self.load_data()
    ...
    def create_widgets(self):
        ...
        tk.Label(form, text="Matricule *:", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.matricule_var = tk.StringVar()
        ...
        if self.personnel_id:
            self.entry_matricule.config(state='readonly')  # empêcher modification du matricule
        row += 1

        tk.Label(form, text="Badge ID :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.badge_var = tk.StringVar()
        tk.Entry(form, textvariable=self.badge_var, width=20).grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Nom *:", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.nom_var = tk.StringVar()
        tk.Entry(form, textvariable=self.nom_var, width=20).grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Prénom *:", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.prenom_var = tk.StringVar()
        tk.Entry(form, textvariable=self.prenom_var, width=20).grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Type de personnel *:", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        ...
        tk.Label(form, text="Date de naissance :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        ...
        tk.Label(form, text="Fonction *:", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.fonction_var = tk.StringVar()
        tk.Entry(form, textvariable=self.fonction_var, width=20).grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Activité :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.activite_var = tk.StringVar()
        self.activite_combo = ttk.Combobox(form, textvariable=self.activite_var, width=18, state='readonly')
        self.activite_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Division :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.division_var = tk.StringVar()
        self.division_combo = ttk.Combobox(form, textvariable=self.division_var, width=18, state='readonly')
        self.division_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Direction *:", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.direction_var = tk.StringVar()
        self.direction_combo = ttk.Combobox(form, textvariable=self.direction_var, width=18, state='readonly')
        self.direction_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Département :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.departement_var = tk.StringVar()
        self.departement_combo = ttk.Combobox(form, textvariable=self.departement_var, width=18, state='readonly')
        self.departement_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Service :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.service_var = tk.StringVar()
        self.service_combo = ttk.Combobox(form, textvariable=self.service_var, width=18, state='readonly')
        self.service_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Équipe :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.equipe_var = tk.StringVar()
        self.equipe_combo = ttk.Combobox(form, textvariable=self.equipe_var, width=18, state='readonly')
        self.equipe_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1

        tk.Label(form, text="Quart :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.quart_var = tk.StringVar()
        self.quart_combo = ttk.Combobox(form, textvariable=self.quart_var, width=18, state='readonly')
        ...
        tk.Label(form, text="Date d'embauche :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        ...
        tk.Label(form, text="Statut :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        ...
        tk.Label(form, text="Concerné par le pointage :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.concerne_var = tk.BooleanVar(value=True)
        tk.Checkbutton(form, variable=self.concerne_var, bg='white').grid(row=row, column=1, sticky='w', pady=5, padx=10)
        row += 1

        tk.Label(form, text="Type de quart :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.type_quart_var = tk.StringVar(value='jour')
        self.quart_type_combo = ttk.Combobox(form, textvariable=self.type_quart_var,
                                             values=['jour', 'nuit', 'personnalisé'],
                                             width=17, state='readonly')
        self.quart_type_combo.grid(row=row, column=1, pady=5, padx=10)
        row += 1
        ...
        tk.Label(form, text="Adresse :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        ...
        tk.Label(form, text="Téléphone :", bg='white').grid(row=row, column=0, sticky='w', pady=5, padx=20)
        self.telephone_var = tk.StringVar()
        tk.Entry(form, textvariable=self.telephone_var, width=20).grid(
            row=row, column=1, sticky='w', pady=5, padx=10)
        row += 1

        tk.Label(form, text="Email :", bg='white').grid(row=row, column=2, sticky='w', pady=5, padx=20)
        ...
        tk.Label(form, text="PHOTO", font=('Arial',12,'bold'),
                 fg='#3498db', bg='white').grid(row=row, column=0, columnspan=4, sticky='w', pady=(10,5))
        row += 1
    ...
    def charger_listes(self):
        try:
            ...
            self.activite_combo['values'] = [a['nom'] for a in activites]
            self.id_maps['activite'] = {a['nom']: a['id'] for a in activites}

            divisions = self.app.db.get_all_divisions()
            self.division_combo['values'] = [d['nom'] for d in divisions]
            self.id_maps['division'] = {d['nom']: d['id'] for d in divisions}

            directions = self.app.db.get_all_directions()
            self.direction_combo['values'] = [d['nom'] for d in directions]
            self.id_maps['direction'] = {d['nom']: d['id'] for d in directions}

            departements = self.app.db.get_all_departements()
            self.departement_combo['values'] = [d['nom'] for d in departements]
            self.id_maps['departement'] = {d['nom']: d['id'] for d in departements}

            services = self.app.db.get_all_services()
            self.service_combo['values'] = [s['nom'] for s in services]
            self.id_maps['service'] = {s['nom']: s['id'] for s in services}

            equipes = self.app.db.get_all_equipes()
            self.equipe_combo['values'] = [e['nom'] for e in equipes]
            self.id_maps['equipe'] = {e['nom']: e['id'] for e in equipes}

            quarts = self.app.db.get_all_quarts()
            self.quart_combo['values'] = [q['nom'] for q in quarts]
            self.id_maps['quart'] = {q['nom']: q['id'] for q in quarts}
        except Exception as e:
    ...
    def load_data(self):
        ...
        if not pers:
            return
        self.matricule_var.set(pers['matricule'])
        self.badge_var.set(pers.get('badge_id', ''))
        self.nom_var.set(pers['nom'])
        self.prenom_var.set(pers['prenom'])
        self.type_person_var.set(pers.get('type_person', 'cadre'))
        if pers.get('date_naissance'):
            self.date_naissance.set_date(datetime.strptime(pers['date_naissance'], '%Y-%m-%d'))
        self.fonction_var.set(pers.get('fonction', ''))
        # Sélectionner les noms à partir des IDs
        if pers.get('activite_id'):
            for nom, id_ in self.id_maps.get('activite', {}).items():
                if id_ == pers['activite_id']:
                    self.activite_var.set(nom)
                    break
        if pers.get('division_id'):
            for nom, id_ in self.id_maps.get('division', {}).items():
                if id_ == pers['division_id']:
                    self.division_var.set(nom)
                    break
        if pers.get('direction_id'):
            for nom, id_ in self.id_maps.get('direction', {}).items():
                if id_ == pers['direction_id']:
                    self.direction_var.set(nom)
                    break
        if pers.get('departement_id'):
            for nom, id_ in self.id_maps.get('departement', {}).items():
                if id_ == pers['departement_id']:
                    self.departement_var.set(nom)
                    break
        if pers.get('service_id'):
            for nom, id_ in self.id_maps.get('service', {}).items():
                if id_ == pers['service_id']:
                    self.service_var.set(nom)
                    break
        if pers.get('equipe_id'):
            for nom, id_ in self.id_maps.get('equipe', {}).items():
                if id_ == pers['equipe_id']:
                    self.equipe_var.set(nom)
                    break
        if pers.get('quart_id'):
            for nom, id_ in self.id_maps.get('quart', {}).items():
                if id_ == pers['quart_id']:
                    self.quart_var.set(nom)
                    break
        if pers.get('date_embauche'):
            self.date_embauche.set_date(datetime.strptime(pers['date_embauche'], '%Y-%m-%d'))
        self.statut_var.set(pers.get('statut', 'actif'))
        self.concerne_var.set(bool(pers.get('concerne_pointage', 1)))
        self.type_quart_var.set(pers.get('type_quart', 'jour'))
        self.heure_entree_var.set(pers.get('heure_entree_theorique', '08:00:00'))
        self.heure_sortie_var.set(pers.get('heure_sortie_theorique', '17:00:00'))
        self.tolerance_entree_var.set(pers.get('tolerance_entree', 10))
        self.tolerance_sortie_var.set(pers.get('tolerance_sortie', 10))
        self.adresse_var.set(pers.get('adresse', ''))
        self.telephone_var.set(pers.get('telephone', ''))
        self.email_var.set(pers.get('email', ''))
        self.photo_actuelle = pers.get('photo')
        if self.photo_actuelle:
            self.photo_path = self.photo_actuelle
            self.afficher_photo(self.photo_actuelle)
    ...
    def save(self):
        ...
        activite_id = self.id_maps.get('activite', {}).get(self.activite_var.get())
        division_id = self.id_maps.get('division', {}).get(self.division_var.get())
        direction_id = self.id_maps.get('direction', {}).get(self.direction_var.get())
        departement_id = self.id_maps.get('departement', {}).get(self.departement_var.get())
        service_id = self.id_maps.get('service', {}).get(self.service_var.get())
        equipe_id = self.id_maps.get('equipe', {}).get(self.equipe_var.get())
        ...
        if hasattr(self, 'photo_path') and self.photo_path:
            ...
            try:
                ...
                photo = self.photo_path  # fallback
        else:
...
class EditProfileDialog:
    ...
    def save(self):
        ...
        try:
            ...
            # Mettre à jour les informations dans l'objet current_user
            self.app.current_user['nom'] = self.nom.get().upper()
            self.app.current_user['prenom'] = self.prenom.get().title()
            self.app.current_user['email'] = self.email.get()
...
class GestionTolerances:
    ...
    def create_tab_individuel(self):
        ...
        tk.Button(btn_frame, text="↩ Réinitialiser", command=self.reset_formulaire,
                  bg='#f39c12', fg='white', width=15).pack(side=tk.LEFT, padx=5)
        # --- NOUVEAU BOUTON POUR APPLIQUER CONCERNE À TOUS ---
        tk.Button(edit_frame, text="👥 Appliquer 'concerné' à tous les agents",
                  command=self.appliquer_concerne_a_tous,
                  bg='#3498db', fg='white', font=('Arial',9,'bold'),
                  width=30, height=1).pack(pady=10)
    
    def appliquer_concerne_a_tous(self):
        if messagebox.askyesno("Confirmation", "Appliquer la valeur 'Concerné par le pointage' à TOUS les agents ?\n\nCocher = tous concernés\nDécocher = tous non concernés"):
            # Demander quelle valeur appliquer
            reponse = messagebox.askquestion("Valeur à appliquer", 
                                         "Cliquez OUI pour que TOUS les agents soient concernés par le pointage.\n"
                                         "Cliquez NON pour qu'aucun ne soit concerné.\n\n"
            ...
            try:
                with self.app.db.get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE personnel SET concerne_pointage = ?", (valeur,))
                    conn.commit()
                messagebox.showinfo("Succès", f"{cursor.rowcount} agents mis à jour.\n'concerné_pointage' = {valeur}")
...
# ============================================================
# CONFIG WEEKEND DIALOG
# ============================================================
class ConfigWeekendDialog:
    def __init__(self, parent, app):
        self.app = app
        self.dialog = tk.Toplevel(parent)
        self.dialog.title("Configuration des jours de weekend")
        self.dialog.geometry("400x300")
        self.dialog.transient(parent)
        self.dialog.grab_set()

        tk.Label(self.dialog, text="Sélectionnez les jours considérés comme weekend :",
        ...
        for i, jour in enumerate(jours):
            ...
            cb = tk.Checkbutton(frame, text=jour, variable=var)
            cb.grid(row=i//3, column=i%3, sticky='w', padx=10, pady=5)

        actuel = self.app.db.get_parametre('jours_weekend', '5,6')
        indices = [int(x.strip()) for x in actuel.split(',') if x.strip()]
        for i in indices:
            if 0 <= i < len(self.vars):
                self.vars[i].set(True)

        btn_frame = tk.Frame(self.dialog)
        btn_frame.pack(pady=20)
        tk.Button(btn_frame, text="Enregistrer", command=self.sauvegarder,
                  bg='#27ae60', fg='white', width=15).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Annuler", command=self.dialog.destroy,
                  bg='#e74c3c', fg='white', width=15).pack(side=tk.LEFT, padx=10)

    def sauvegarder(self):
        indices = [str(i) for i, var in enumerate(self.vars) if var.get()]
        nouvelle_valeur = ','.join(indices) if indices else ''
        self.app.db.set_parametre('jours_weekend', nouvelle_valeur)
        messagebox.showinfo("Succès", "Configuration des jours de weekend enregistrée.")
        self.dialog.destroy()
...
class ExportDialog:
    ...
    def ouvrir(self):
        self.window = tk.Toplevel(self.parent)
        self.window.title("Exporter des données")
        self.window.geometry("600x500")
        self.window.transient(self.parent)
        self.window.grab_set()
        self.window.configure(bg='#f0f0f0')

        tk.Label(self.window, text="📤 EXPORT DES DONNÉES", font=('Arial',16,'bold'),
        ...
        # Type de données
        tk.Label(main, text="Type de données :", font=('Arial',10,'bold'), bg='white').pack(anchor='w', padx=10, pady=5)
        self.type_var = tk.StringVar(value='personnel')
        types = [('👥 Personnel', 'personnel'),
                 ('⏱️ Pointages', 'pointages'),
                 ('⚠️ Retards cumulés', 'retards'),
                 ('📅 Absences', 'absences'),
                 ('📋 Présence', 'presence')]
        for txt, val in types:
            tk.Radiobutton(main, text=txt, variable=self.type_var, value=val,
                           bg='white', anchor='w').pack(anchor='w', padx=20, pady=2)

        # Frame pour les options spécifiques
        self.options_frame = tk.LabelFrame(main, text="Options", font=('Arial',10,'bold'), bg='white')
        self.options_frame.pack(fill=tk.X, padx=10, pady=10)
        self.update_options()

        self.type_var.trace('w', lambda *a: self.update_options())

        # Format
        tk.Label(main, text="Format :", font=('Arial',10,'bold'), bg='white').pack(anchor='w', padx=10, pady=5)
        self.format_var = tk.StringVar(value='csv')
        fmts = [('CSV', 'csv'), ('Excel', 'excel'), ('PDF', 'pdf')]
        for txt, val in fmts:
            tk.Radiobutton(main, text=txt, variable=self.format_var, value=val,
                           bg='white').pack(anchor='w', padx=20, pady=2, side=tk.LEFT)

        # Boutons
        btn_frame = tk.Frame(main, bg='white')
        btn_frame.pack(pady=20)
        tk.Button(btn_frame, text="✅ Exporter", command=self.exporter,
                  bg='#27ae60', fg='white', font=('Arial',12,'bold'),
                  width=15, height=2).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="❌ Annuler", command=self.window.destroy,
                  bg='#e74c3c', fg='white', width=15).pack(side=tk.LEFT, padx=10)
    ...
    def update_options(self):
        ...
        if typ == 'pointages':
            self.create_periode_options()
        elif typ == 'retards':
            self.create_periode_options(avec_filtre_agent=True)
        elif typ == 'absences':
            self.create_periode_options(avec_filtres_hierarchie=True)
        elif typ == 'presence':
            self.create_periode_options(avec_direction=True)
        else:
            tk.Label(self.options_frame, text="(Aucune option requise)", bg='white').pack(pady=10)

    def create_periode_options(self, avec_direction=False, avec_filtre_agent=False, avec_filtres_hierarchie=False):
        row = 0
        tk.Label(self.options_frame, text="Période du :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
        self.date_debut = DateEntry(self.options_frame, width=12, date_pattern='yyyy-mm-dd')
        self.date_debut.grid(row=row, column=1, padx=5, pady=5)
        self.date_debut.set_date(date.today().replace(day=1))
        tk.Label(self.options_frame, text="au :", bg='white').grid(row=row, column=2, padx=5, pady=5)
        self.date_fin = DateEntry(self.options_frame, width=12, date_pattern='yyyy-mm-dd')
        self.date_fin.grid(row=row, column=3, padx=5, pady=5)
        self.date_fin.set_date(date.today())
        row += 1

        if avec_direction:
            tk.Label(self.options_frame, text="Direction :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
            self.direction_var = tk.StringVar()
            self.direction_combo = ttk.Combobox(self.options_frame, textvariable=self.direction_var, width=25)
            self.direction_combo.grid(row=row, column=1, columnspan=3, padx=5, pady=5, sticky='w')
            self.charger_directions()
            self.direction_combo.set('Toutes')
            row += 1

        if avec_filtre_agent:
            tk.Label(self.options_frame, text="Agent :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
            self.agent_var = tk.StringVar()
            self.agent_combo = ttk.Combobox(self.options_frame, textvariable=self.agent_var, width=25)
            self.agent_combo.grid(row=row, column=1, columnspan=3, padx=5, pady=5, sticky='w')
            self.charger_agents()
            self.agent_combo.set('Tous')
            row += 1

        if avec_filtres_hierarchie:
            tk.Label(self.options_frame, text="Direction :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
            self.direction_var = tk.StringVar()
            self.direction_combo = ttk.Combobox(self.options_frame, textvariable=self.direction_var, width=20)
            self.direction_combo.grid(row=row, column=1, padx=5, pady=5, sticky='w')
            self.charger_directions()
            self.direction_combo.set('Toutes')
            row += 1

            tk.Label(self.options_frame, text="Département :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
            self.departement_var = tk.StringVar()
            self.departement_combo = ttk.Combobox(self.options_frame, textvariable=self.departement_var, width=20)
            self.departement_combo.grid(row=row, column=1, padx=5, pady=5, sticky='w')
            self.charger_departements()
            self.departement_combo.set('Tous')
            row += 1

            tk.Label(self.options_frame, text="Service :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
            self.service_var = tk.StringVar()
            self.service_combo = ttk.Combobox(self.options_frame, textvariable=self.service_var, width=20)
            self.service_combo.grid(row=row, column=1, padx=5, pady=5, sticky='w')
            self.charger_services()
            self.service_combo.set('Tous')
            row += 1

            tk.Label(self.options_frame, text="Équipe :", bg='white').grid(row=row, column=0, padx=5, pady=5, sticky='w')
            self.equipe_var = tk.StringVar()
            self.equipe_combo = ttk.Combobox(self.options_frame, textvariable=self.equipe_var, width=20)
            self.equipe_combo.grid(row=row, column=1, padx=5, pady=5, sticky='w')
            self.charger_equipes()
            self.equipe_combo.set('Toutes')
            row += 1
    ...
    def charger_agents(self):
        try:
            ...
            if hasattr(self, 'agent_combo'):
                self.agent_combo['values'] = ['Tous'] + [f"{a['prenom']} {a['nom']} ({a['matricule']})" for a in agents if a.get('statut')=='actif']
        except:
            pass
    ...
    def charger_departements(self):
        try:
            depts = self.app.db.get_all_departements()
            if hasattr(self, 'departement_combo'):
                self.departement_combo['values'] = ['Tous'] + [d['nom'] for d in depts]
        except:
            pass
    ...
    def exporter(self):
        ...
        data = None
        if typ == 'personnel':
            data = self.get_personnel_data()
        elif typ == 'pointages':
            data = self.get_pointages_data()
        elif typ == 'retards':
            data = self.get_retards_data()
        elif typ == 'absences':
            data = self.get_absences_data()
        elif typ == 'presence':
            data = self.get_presence_data()
        ...
        try:
            ...
            messagebox.showinfo("Succès", f"Export terminé vers {filename}")
            self.window.destroy()
        except Exception as e:
            messagebox.showerror("Erreur", f"Erreur lors de l'export : {e}")

    def get_personnel_data(self):
        personnel = self.app.db.get_personnel()
        if not personnel:
            return None
        df = pd.DataFrame([dict(p) for p in personnel])
        cols = ['matricule', 'badge_id', 'nom', 'prenom', 'type_person', 'fonction',
                'direction_nom', 'departement_nom', 'service_nom', 'equipe_nom', 'statut',
    ...
    def get_pointages_data(self):
        date_debut = self.date_debut.get_date().strftime('%Y-%m-%d')
        date_fin = self.date_fin.get_date().strftime('%Y-%m-%d')
        pointages = self.app.db.get_pointages_filtres(date_debut=date_debut, date_fin=date_fin)
        if not pointages:
            return None
        df = pd.DataFrame([dict(p) for p in pointages])
        cols = ['id', 'matricule', 'type_pointage', 'date_pointage', 'heure_pointage',
                'mode', 'justification', 'minutes_retard', 'est_weekend', 'est_jour_ferie']
        cols_present = [c for c in cols if c in df.columns]
        return df[cols_present]

    def get_retards_data(self):
        date_debut = self.date_debut.get_date().strftime('%Y-%m-%d')
        date_fin = self.date_fin.get_date().strftime('%Y-%m-%d')
        direction = None
        if hasattr(self, 'direction_combo') and self.direction_combo.get() != 'Toutes':
            direction = self.direction_combo.get()
        agent_sel = None
        if hasattr(self, 'agent_combo'):
            agent_sel = self.agent_combo.get()
        import re
        matricule = None
        if agent_sel and agent_sel != 'Tous':
            match = re.search(r'\(([^)]+)\)', agent_sel)
            if match:
                matricule = match.group(1)
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            query = '''
                SELECT r.id, r.matricule, p.nom, p.prenom, r.date_retard, r.minutes_retard,
                       r.est_justifie, r.justification,
                       dir.nom_direction as direction
                FROM retards_cumules r
                JOIN personnel p ON r.personnel_id = p.id
                LEFT JOIN directions dir ON p.direction_id = dir.id
    ...
    def get_absences_data(self):
        ...
        if hasattr(self, 'direction_combo') and self.direction_combo.get() != 'Toutes':
            direction = self.direction_combo.get()
        departement = None
        if hasattr(self, 'departement_combo') and self.departement_combo.get() != 'Tous':
            departement = self.departement_combo.get()
        service = None
        if hasattr(self, 'service_combo') and self.service_combo.get() != 'Tous':
            service = self.service_combo.get()
        equipe = None
        if hasattr(self, 'equipe_combo') and self.equipe_combo.get() != 'Toutes':
            equipe = self.equipe_combo.get()
        ...
        def est_ferie(d):
            with self.app.db.get_connection() as conn2:
                cur = conn2.cursor()
                cur.execute("SELECT COUNT(*) FROM jours_feries WHERE date_jour=?", (d,))
                return cur.fetchone()[0] > 0

        resultats = []
        jour = date_debut
        while jour <= date_fin:
            if not est_weekend(jour) and not est_ferie(jour):
                for a in agents:
                    c.execute("SELECT COUNT(*) FROM pointages WHERE personnel_id=? AND date_pointage=?", (a['id'], jour))
                    a_pointe = c.fetchone()[0] > 0
                    c.execute('''
                        SELECT COUNT(*) FROM conges WHERE personnel_id=? AND statut='approuve'
                        AND date_debut <= ? AND date_fin >= ?
                    ''', (a['id'], jour, jour))
                    a_conge = c.fetchone()[0] > 0
                    if not a_pointe and not a_conge:
                        resultats.append({
    ...
    def get_presence_data(self):
        ...
        if hasattr(self, 'direction_combo') and self.direction_combo.get() != 'Toutes':
            direction = self.direction_combo.get()
        rapport = self.app.db.get_presence_report(date_debut, date_fin, direction_nom=direction)
        if not rapport:
            return None
        df = pd.DataFrame([dict(r) for r in rapport])
        return df
    ...
    def export_pdf(self, df, filename):
        ...
        styles = getSampleStyleSheet()
        title_style = styles['Title']
        elements.append(Paragraph(f"Export des données - {self.type_var.get()}", title_style))
        elements.append(Spacer(1, 0.5*cm))
        data = [df.columns.tolist()] + df.values.tolist()
...
class GestionPointages:
    ...
    def charger_directions(self):
        try:
            dirs = self.app.db.get_all_directions()
            self.filtre_dir['values'] = [''] + [d['nom'] for d in dirs]
        except:
            pass
    ...
    def appliquer_filtres(self):
        self.filtres = {
            ...
            'direction_nom': self.filtre_dir.get().strip() or None
        }
        self.charger_pointages()
    ...
    def charger_pointages(self):
        ...
        try:
            ...
            for p in pts:
                retard = f"{p.get('minutes_retard', 0)} min" if p.get('minutes_retard', 0) else '-'
                heure = p['heure_pointage'][:5] if p.get('heure_pointage') else ''
                self.tree.insert('','end', values=(
                    p['id'], p['date_pointage'], heure,
                    p['matricule'], f"{p.get('prenom', '')} {p.get('nom', '')}",
                    p['type_pointage'], p.get('quart_nom', ''),
                    retard, p.get('justification', '') or '', p.get('mode', '')
                ))
        except Exception as e:
...
class GestionHeuresSup:
    ...
    def charger_liste(self):
        ...
        for h in hs_list:
            self.tree.insert('','end', values=(
                h['id'], h['matricule'], f"{h.get('prenom', '')} {h.get('nom', '')}",
...
# ============================================================
# GESTION CONGÉS (ABSENCES) - VERSION CORRIGÉE
# ============================================================
class GestionAbsences:
    ...
    def charger_liste_matricules(self):
        try:
            ...
            matricules = [p['matricule'] for p in pers if p.get('statut') == 'actif']
...
class MainWindow:
    ...
    def setup_menu(self):
        ...
        rapp_menu.add_command(label="📅 Absences", command=self.show_rapport_absences)
        rapp_menu.add_command(label="⚠️ Retards", command=self.show_rapport_retards)
        ...
        if self.app.current_user['role'] == 'admin':
            ...
            params_menu.add_command(label="🗓️ Jours de weekend", command=self.configurer_weekend)
    ...
    def create_widgets(self):
        ...
        try:
            ...
            if os.path.exists(logo_path):
                img = Image.open(logo_path)
                img = img.resize((110, 50), Image.LANCZOS)
                self.logo_image = ImageTk.PhotoImage(img)
    ...
    def configurer_weekend(self):
        ConfigWeekendDialog(self.root, self.app)
    ...
    def show_dashboard(self):
        ...
        for i, (title, value, color) in enumerate(cards):
            ...
            tk.Label(card, text=str(value), font=('Arial', 20, 'bold'), bg=color, fg='white').pack(expand=True)
        ...
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            c.execute("""
                SELECT dir.nom_direction, COUNT(p.id)
                FROM personnel p
                LEFT JOIN directions dir ON p.direction_id = dir.id
                WHERE p.statut='actif'
                GROUP BY dir.nom_direction
            """)
            directions = {row[0] or 'Sans direction': row[1] for row in c.fetchall()}
        if directions:
        ...
        # Filtre rapide personnel
        filter_section = tk.LabelFrame(scrollable_frame, text="🔍 Recherche rapide du personnel",
                                font=('Arial', 12, 'bold'), bg='white')
        filter_section.pack(fill=tk.X, padx=20, pady=10)
        filtres_frame = tk.Frame(filter_section, bg='white')
        filtres_frame.pack(fill=tk.X, padx=10, pady=5)
        
        tk.Label(filtres_frame, text="Recherche (nom, matricule) :", bg='white').grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.dash_search = tk.Entry(filtres_frame, width=20)
        self.dash_search.grid(row=0, column=1, padx=5, pady=5)
        
        tk.Label(filtres_frame, text="Direction :", bg='white').grid(row=0, column=2, padx=5, pady=5, sticky='w')
        self.dash_direction = ttk.Combobox(filtres_frame, width=18)
        self.dash_direction.grid(row=0, column=3, padx=5, pady=5)
        
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            c.execute("SELECT DISTINCT dir.nom_direction FROM personnel p LEFT JOIN directions dir ON p.direction_id = dir.id WHERE dir.nom_direction IS NOT NULL ORDER BY dir.nom_direction")
            dirs = [row[0] for row in c.fetchall()]
        self.dash_direction['values'] = ['Toutes'] + dirs
        self.dash_direction.set('Toutes')
        
        tk.Label(filtres_frame, text="Département :", bg='white').grid(row=0, column=4, padx=5, pady=5, sticky='w')
        self.dash_departement = ttk.Combobox(filtres_frame, width=18)
        self.dash_departement.grid(row=0, column=5, padx=5, pady=5)
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            c.execute("SELECT DISTINCT dep.nom_departement FROM personnel p LEFT JOIN departements dep ON p.departement_id = dep.id WHERE dep.nom_departement IS NOT NULL ORDER BY dep.nom_departement")
            depts = [row[0] for row in c.fetchall()]
        self.dash_departement['values'] = ['Tous'] + depts
        self.dash_departement.set('Tous')
        
        tk.Label(filtres_frame, text="Service :", bg='white').grid(row=0, column=6, padx=5, pady=5, sticky='w')
        self.dash_service = ttk.Combobox(filtres_frame, width=18)
        self.dash_service.grid(row=0, column=7, padx=5, pady=5)
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            c.execute("SELECT DISTINCT s.nom_service FROM personnel p LEFT JOIN services s ON p.service_id = s.id WHERE s.nom_service IS NOT NULL ORDER BY s.nom_service")
            servs = [row[0] for row in c.fetchall()]
        self.dash_service['values'] = ['Tous'] + servs
        self.dash_service.set('Tous')
        
        tk.Label(filtres_frame, text="Équipe :", bg='white').grid(row=0, column=8, padx=5, pady=5, sticky='w')
        self.dash_equipe = ttk.Combobox(filtres_frame, width=18)
        self.dash_equipe.grid(row=0, column=9, padx=5, pady=5)
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            c.execute("SELECT DISTINCT eq.nom_equipe FROM personnel p LEFT JOIN equipes eq ON p.equipe_id = eq.id WHERE eq.nom_equipe IS NOT NULL ORDER BY eq.nom_equipe")
            eqs = [row[0] for row in c.fetchall()]
        self.dash_equipe['values'] = ['Toutes'] + eqs
        self.dash_equipe.set('Toutes')
        
        tk.Button(filtres_frame, text="🔍 Filtrer", command=self.filtrer_personnel_dashboard,
                  bg='#3498db', fg='white', font=('Arial', 9, 'bold')).grid(row=0, column=10, padx=10, pady=5)

        tree_frame = tk.Frame(filter_section, bg='white')
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        columns = ('Matricule', 'Nom', 'Prénom', 'Fonction', 'Direction', 'Département', 'Service', 'Équipe', 'Statut')
        self.dash_tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=8)
        for col in columns:
            self.dash_tree.heading(col, text=col)
            self.dash_tree.column(col, width=100)
        self.dash_tree.column('Nom', width=120)
        self.dash_tree.column('Prénom', width=120)
        ...
        vsb.pack(side=tk.RIGHT, fill=tk.Y)
        self.filtrer_personnel_dashboard()
        ...
        cols = ('Matricule', 'Nom', 'Prénom', 'Direction', 'Nb retards', 'Total minutes')
        ...
        with self.app.db.get_connection() as conn:
            c = conn.cursor()
            c.execute("""
                SELECT r.matricule, p.nom, p.prenom, dir.nom_direction as direction,
                       COUNT(r.id) as nb_retards,
                       SUM(r.minutes_retard) as total_min
                FROM retards_cumules r
                JOIN personnel p ON r.personnel_id = p.id
                ...
                LIMIT 5
            """, (premier_jour, aujourdhui))
            for row in c.fetchall():
                tree.insert('', 'end', values=(
                    row['matricule'], row['nom'], row['prenom'], row['direction'] or '',
        ...
        cols2 = ('Date', 'Heure', 'Matricule', 'Nom', 'Type', 'Retard')
        tree2 = ttk.Treeview(last_frame, columns=cols2, show='headings', height=8)
        for col in cols2:
        ...
        for pt in derniers[:15]:
            retard = f"{pt.get('minutes_retard', 0)} min" if pt.get('minutes_retard', 0) > 0 else "-"
    ...
    def show_personnel(self):
        # Ajoutez ces attributs dans __init__ de MainWindow (ou dans show_personnel)
        self.personnel_page = 0          # page courante (0-index)
        self.personnel_per_page = 20     # lignes par page
        self.personnel_total = 0
        self.personnel_data = []         # toutes les données du personnel
        self.clear_content()
        ...
        cols = ('ID','Matricule','Badge','Photo','Nom','Prénom','Type','Fonction','Direction','Département','Statut')
        # Dans show_personnel, après la création de self.tree_pers, ajoutez un cadre pour les contrôles de pagination
        control_frame = tk.Frame(self.content_frame, bg='#ecf0f1')
        control_frame.pack(fill=tk.X, padx=20, pady=5)

        tk.Label(control_frame, text="Lignes par page :", bg='#ecf0f1').pack(side=tk.LEFT, padx=5)
        self.personnel_page_size = tk.StringVar(value="20")
        page_size_spin = tk.Spinbox(control_frame, from_=5, to=200, textvariable=self.personnel_page_size, width=5, command=self.charger_personnel_page)
        page_size_spin.pack(side=tk.LEFT, padx=5)

        self.personnel_prev_btn = tk.Button(control_frame, text="◀ Précédent", command=self.personnel_page_prev, state=tk.DISABLED)
        self.personnel_prev_btn.pack(side=tk.LEFT, padx=5)
        self.personnel_next_btn = tk.Button(control_frame, text="Suivant ▶", command=self.personnel_page_next, state=tk.DISABLED)
        self.personnel_next_btn.pack(side=tk.LEFT, padx=5)
        self.personnel_page_label = tk.Label(control_frame, text="Page 1 / 1", bg='#ecf0f1')
        self.personnel_page_label.pack(side=tk.LEFT, padx=10)
        self.tree_pers = ttk.Treeview(tree_f, columns=cols, show='headings', height=20)
        wids = [50,100,100,120,120,100,150,120,120,80,80]
        for c,w in zip(cols,wids):
        ...
        vsb.pack(side=tk.RIGHT, fill=tk.Y)
        self.tree_pers.bind('<Double-Button-1>', self.on_double_click_personnel)
        self.charger_personnel_page()
    ...
    def charger_personnel(self):
        ...
        for p in pers:
            self.tree_pers.insert('','end', values=(
                p['id'], p['matricule'], p.get('badge_id', '-'),
                p.get('photo', '-'), p['nom'], p['prenom'], p.get('type_person', '-'),
                p.get('fonction', ''),
                p.get('direction_nom', ''),
                p.get('departement_nom', '-'),
                p.get('statut', 'actif')
            ))
    #----------------------------------------
    def charger_personnel_page(self):
        # Récupérer toutes les données (avec filtres éventuels)
        # Vous pouvez conserver la recherche existante
        search = self.search_entry.get().strip() if hasattr(self, 'search_entry') else ''
        if search:
            self.personnel_data = self.app.db.search_personnel(search)
        else:
            self.personnel_data = self.app.db.get_personnel()
        self.personnel_total = len(self.personnel_data)
        self.personnel_page = 0
        self.afficher_personnel_page()

    def afficher_personnel_page(self):
        # Vider l'arbre
        for i in self.tree_pers.get_children():
            self.tree_pers.delete(i)
        # Calculer les indices de début et fin
        per_page = int(self.personnel_page_size.get())
        start = self.personnel_page * per_page
        end = min(start + per_page, self.personnel_total)
        # Insérer les lignes
        for p in self.personnel_data[start:end]:
            self.tree_pers.insert('', 'end', values=(
                p['id'], p['matricule'], p.get('badge_id', '-'),
                p.get('photo', '-'), p['nom'], p['prenom'], p.get('type_person', '-'),
                p.get('fonction', ''),
        ...
        # Mettre à jour les boutons et l'étiquette
        total_pages = (self.personnel_total + per_page - 1) // per_page if self.personnel_total > 0 else 1
        self.personnel_page_label.config(text=f"Page {self.personnel_page+1} / {total_pages}")
        self.personnel_prev_btn.config(state=tk.NORMAL if self.personnel_page > 0 else tk.DISABLED)
        self.personnel_next_btn.config(state=tk.NORMAL if (self.personnel_page+1)*per_page < self.personnel_total else tk.DISABLED)

    def personnel_page_prev(self):
        if self.personnel_page > 0:
            self.personnel_page -= 1
            self.afficher_personnel_page()

    def personnel_page_next(self):
        per_page = int(self.personnel_page_size.get())
        if (self.personnel_page+1)*per_page < self.personnel_total:
            self.personnel_page += 1
            self.afficher_personnel_page()
#----------------------------------------
    ...
    def on_double_click_personnel(self, event):
        sel = self.tree_pers.selection()
        if sel:
            pid = self.tree_pers.item(sel[0])['values'][0]
            self.detail_personnel(pid)

    def detail_personnel(self, pid):
        pers = self.app.db.get_personnel(pid)
        if not pers:
            messagebox.showerror("Erreur", "Personnel introuvable")
            return
        win = tk.Toplevel(self.root)
        win.title(f"Détail - {pers['prenom']} {pers['nom']}")
        ...
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        header = tk.Frame(scrollable, bg='white', relief=tk.GROOVE, bd=2)
        header.pack(fill=tk.X, padx=20, pady=20)
        photo_frame = tk.Frame(header, bg='#ecf0f1', width=180, height=180)
        photo_frame.pack_propagate(False)
        photo_frame.pack(side=tk.LEFT, padx=20, pady=20)
        photo_label = tk.Label(photo_frame, bg='#ecf0f1', text="📷", font=('Arial', 50))
        photo_label.pack(expand=True)
        if pers.get('photo') and os.path.exists(pers['photo']):
            try:
                img = Image.open(pers['photo'])
        ...
        info.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=20, pady=20)
        tk.Label(info, text=f"{pers['prenom']} {pers['nom']}", font=('Arial', 20, 'bold'),
                 bg='white', fg='#2c3e50').pack(anchor='w')
        tk.Label(info, text=f"Matricule : {pers['matricule']}", font=('Arial', 12),
                 bg='white').pack(anchor='w', pady=(10,0))
        tk.Label(info, text=f"Fonction : {pers['fonction']}", bg='white').pack(anchor='w')
        tk.Label(info, text=f"Direction : {pers.get('direction_nom', '-')}", bg='white').pack(anchor='w')
        tk.Label(info, text=f"Service : {pers.get('service_nom', '-')}", bg='white').pack(anchor='w')
        tk.Label(info, text=f"Statut : {pers['statut']}", bg='white').pack(anchor='w')
        details = tk.LabelFrame(scrollable, text="Informations détaillées",
                                font=('Arial', 14, 'bold'), bg='white', fg='#3498db')
        details.pack(fill=tk.X, padx=20, pady=10)
        lignes = [
            ("Badge ID", pers.get('badge_id', '-')),
            ("Type personnel", pers.get('type_person', '-')),
            ("Département", pers.get('departement_nom', '-')),
            ("Équipe", pers.get('equipe_nom', '-')),
            ("Date naissance", pers.get('date_naissance', '-')),
            ("Date embauche", pers.get('date_embauche', '-')),
            ("Adresse", pers.get('adresse', '-')),
            ("Téléphone", pers.get('telephone', '-')),
            ("Email", pers.get('email', '-')),
            ("Heure entrée théorique", pers.get('heure_entree_theorique', '08:00:00')),
            ("Heure sortie théorique", pers.get('heure_sortie_theorique', '17:00:00')),
            ("Tolérance entrée", f"{pers.get('tolerance_entree', 10)} min"),
            ("Tolérance sortie", f"{pers.get('tolerance_sortie', 10)} min"),
            ("Type de quart", pers.get('type_quart', 'jour')),
            ("Concerné pointage", "Oui" if pers.get('concerne_pointage') else "Non"),
        ]
        for i, (label, valeur) in enumerate(lignes):
            ...
            tk.Label(details, text=valeur, bg='white', anchor='w').grid(row=i, column=1, sticky='w', padx=10, pady=3)
        pointages_frame = tk.LabelFrame(scrollable, text="Derniers pointages",
                                        font=('Arial', 14, 'bold'), bg='white', fg='#3498db')
        pointages_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)
        pointages = self.app.db.get_pointages_filtres(matricule=pers['matricule'])
        if pointages:
            text_widget = tk.Text(pointages_frame, height=8, width=80, font=('Courier', 9))
            text_widget.pack(padx=5, pady=5, fill=tk.BOTH, expand=True)
            text_widget.insert(tk.END, f"{'Date':<12} {'Heure':<8} {'Type':<12} {'Mode':<10} {'Retard':<10}\n")
            text_widget.insert(tk.END, "-"*60 + "\n")
            for p in pointages:
                retard = f"{p.get('minutes_retard', 0)} min" if p.get('minutes_retard', 0) else "-"
                ligne = f"{p['date_pointage']:<12} {p['heure_pointage'][:5]:<8} {p['type_pointage']:<12} {p['mode']:<10} {retard:<10}\n"
                text_widget.insert(tk.END, ligne)
            text_widget.config(state=tk.DISABLED)
        else:
            tk.Label(pointages_frame, text="Aucun pointage enregistré.", bg='white').pack(pady=20)
        tk.Button(win, text="Fermer", command=win.destroy,
                  bg='#e74c3c', fg='white', font=('Arial', 10, 'bold'),
                  width=15, height=1).pack(pady=15)

    # ============================================================
    # AUTRES MÉTHODES (appel des autres classes)
    # ============================================================
    ...
    def show_rapport_retards(self):
        rr = RapportRetards(self.root, self.app)
        rr.ouvrir()

    def show_rapport_presence(self):
        rp = RapportPresence(self.root, self.app)
        rp.ouvrir()

    def show_rapport_absences(self):
        ra = RapportAbsences(self.root, self.app)
        ra.ouvrir()

    def show_rapport_hs(self):
        messagebox.showinfo("Info", "Utilisez le menu Gestion > Heures supplémentaires")
    ...
    def show_about(self):
        ...
        ✓ Rapports complets avec export PDF
    ...
    def importer_pointages(self):
        try:
            ...
            messagebox.showerror("Erreur", f"Impossible d'ouvrir l'importateur:\n{str(e)}")
            import traceback
            traceback.print_exc()

    def exporter_donnees(self):
        ed = ExportDialog(self.root, self.app)
        ed.ouvrir()

    def filtrer_personnel_dashboard(self):
        search = self.dash_search.get().strip()
        direction = self.dash_direction.get()
        if direction == 'Toutes':
            direction = None
        departement = self.dash_departement.get()
        if departement == 'Tous':
            departement = None
        service = self.dash_service.get()
        if service == 'Tous':
            service = None
        equipe = self.dash_equipe.get()
        if equipe == 'Toutes':
            equipe = None

        query = """
            SELECT p.matricule, p.nom, p.prenom, p.fonction,
                   dir.nom_direction as direction,
                   dep.nom_departement as departement,
                   s.nom_service as service,
                   e.nom_equipe as equipe,
        ...
        for r in rows:
            self.dash_tree.insert('', 'end', values=(
                r['matricule'], r['nom'], r['prenom'], r['fonction'],
                r['direction'] or '', r['departement'] or '', r['service'] or '', r['equipe'] or '',
                r['statut']
            ))


# ============================================================
# CLASSE PRINCIPALE (PointageApp)
# ============================================================
class PointageApp:
    def __init__(self):
        self.db = Database()
...
if __name__ == "__main__":
    try:
        print("="*60)
        print("🚀 SYSTÈME DE POINTAGE - VERSION ULTIME")
        print("="*60)
        pkgs = ['tkcalendar', 'matplotlib', 'pandas', 'pillow', 'openpyxl', 'reportlab', 'pdf_reports']
